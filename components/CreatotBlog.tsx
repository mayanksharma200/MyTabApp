...
export default function CreatorBlog({ selectedType }: CreatorBlogProps) {
  ...
  const fetchOffers = async () => {
    try {
      ...
      const res = await axios.get(
        `https://flashoffers-ui.vercel.app/api/posts/blog-content${queryType}`
      );

      if (res.data && res.data.articles) {
        // Assuming imageDataUrl is provided or you can adapt this part
        setOffers(res.data.articles);
      } else {
        setOffers([]);
      }
    } catch (err) {
  ...
  const filteredOffers =
    selectedType === "hot"
      ...
      : offers.filter(
          (offer) =>
            offer.type && offer.type.toLowerCase() === selectedType.toLowerCase()
        );

  const visibleOffers = filteredOffers.slice(0, visibleCount);

  const handleShowMore = () =>
    setVisibleCount((prev) => Math.min(prev + BATCH_SIZE, filteredOffers.length));
  ...
  if (loading) {
    return (
      <View style={styles.centered}>
        ...
        <Text style={{ marginTop: 12, color: "#D97706" }}>Loading offers...</Text>
      </View>
    );
  }
