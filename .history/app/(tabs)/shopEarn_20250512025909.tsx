...
import { setToken } from "../../store/authSlice"; // Adjust path to your authSlice
...
export default function ShopEarn() {
  ...
  // Load token from SecureStore into Redux on mount if missing
  useEffect(() => {
  ...
  const fetchData = async (authToken: string) => {
    try {
      ...
      console.log("Fetching ShopEarn data with token:", authToken);
  ...
  // Fetch data when token changes and is present
  useEffect(() => {
    if (token) {
      fetchData(token);
    }
  ...
  const handleRedeem = async () => {
    ...
    try {
      ...
      // Refresh rewards and redemptions after redeem
