...
export default function ShopEarn() {
    ...
    const fetchData = async () => {
      try {
        ...
        const offersRes = await axios.get(
          "http://192.168.1.7:9000/posts/shopEarn/offers",
          { headers: { Authorization: `Bearer ${token}` } }
        );

        const rewardsRes = await axios.get(
          "http://192.168.1.7:9000/shopEarn/rewards",
          { headers: { Authorization: `Bearer ${token}` } }
        );

        const redemptionsRes = await axios.get(
          "http://192.168.1.7:9000/shopEarn/redemptions",
          { headers: { Authorization: `Bearer ${token}` } }
        );
  ...
  const handleViewOffer = async (offer: Offer) => {
    try {
      await axios.post(
        "http://192.168.1.7:9000/shopEarn/trackClick",
        { offerId: offer._id },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleRedeem = async () => {
    ...
    try {
      await axios.post(
        "http://192.168.1.7:9000/shopEarn/redeem",
        {
      ...
      const [rewardsRes, redemptionsRes] = await Promise.all([
        axios.get("http://192.168.1.7:9000/shopEarn/rewards", {
          headers: { Authorization: `Bearer ${token}` },
        }),
        axios.get("http://192.168.1.7:9000/shopEarn/redemptions", {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);
